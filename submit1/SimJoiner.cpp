    ...
    for (int id2 = 0; id2 < (int)words2.size(); id2++) {
        ...
        for(int j = 0; j < prefix; ++ j) {
            ...
            for(auto & id1 : invertedListJac[sub]) {
                int length2 = words1[id1.first].size();
                if (length2 >= tau * length) {
                    int threshold = ceil(tau * (length + length2) / (1 + tau));
                    int ubound = min(length - j, length2 - int(id1.second));
                    if (m[id1.first] + ubound >= threshold)
                        m[id1.first] += 1;
        ...
        for (auto & i : m) {
            if(i.second > 0)
                candidatesJac.push_back(make_pair(i.first, id2));
        }
        ...
        //             if (ubound >= threshold)
        //                 candidatesJac.push_back(make_pair(id1.first, id2));
        //         }
        //     }
        // }
        for (auto& id1 : smallstr) {
